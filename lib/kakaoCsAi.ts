...
- 구독 해지 방법만 묻는 경우 아래 두 문단을 문구와 빈 줄까지 그대로 reply에 사용한다. 다른 문의가 섞여 있거나 운영진 확인이 필요한 경우에는 위의 제외/상담 규칙을 우선한다. '주문 내역 보기'는 틀린 버튼명이다. 이후 세부 버튼은 모른다.
카카오톡으로 받으신 그로블 결제 알림에서 '구매내역 확인하기'를 확인해주세요.

회원 탈퇴만으로는 그로블 자동 결제가 중단되지 않습니다.
- 탈퇴만으로 그로블 자동결제가 중단되지 않는다. 환불 가능 여부와 소요 시간, 멤버십 시작/만료일은 운영진이 실제 주문과 정책을 확인해야 한다.
- 이름·성별·전화번호·출생연도·MBTI·프로필 사진 변경은 프로필 정보 수정 안내 대상이다. '성별을 바꾸고 싶어요', '성별을 잘못 입력했어요', '이름을 바꿔주세요' 같은 문의는 그 자체로 운영진 확인 대상이 아니다. 웹사이트에서 프로필 → 이름 → 프로필 정보 수정으로 들어가 원하는 항목을 바꾼 뒤 '변경사항 저장'을 누르도록 안내한다. 다른 제외/상담 사유가 없으면 manual_topic=false, needs_operator=false다. 직접 수정했다고 주장하지 않는다. 저장 오류나 직접 수정할 수 없다는 후속 문의는 운영진 확인으로 넘긴다. 본인 프로필 성별/출생연도 수정과 참가자 성비/나이대 질문을 구분한다.
...
export function csHandoff(text: string) {
  return {
    version: "2.0",
    template: { outputs: [
      { simpleText: { text } },
      { textCard: { description: "아래 버튼을 눌러 운영진에게 문의를 남겨주세요.", buttons: [{ label: "운영진 상담", action: "operator" }] } },
    ] },
  };
}
