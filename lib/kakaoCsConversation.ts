...
import { CS_FALLBACK, CsMessage, csText, csHandoff, generateCsReply } from "./kakaoCsAi";
...
export async function prepareCsConversation(input: SkillIdentity) {
  ...
  if (data.status !== "claimed") return { response: data.status === "busy"
    ? csText("앞서 보내주신 문의의 답변을 준비 중이에요. 답변 후 이어서 남겨주세요.")
    : csHandoff(handoff) };
  ...
  // Persist the incoming message before model/callback work: a failed reply must
  // not erase the customer's request, including when the background job stops.
  const { error: receivedError } = await admin.from("kakao_cs_conversations").update({
    messages: messages.slice(-12), updated_at: new Date().toISOString(),
  }).eq("session_key", session).eq("lease_token", token);
  if (receivedError) throw new Error("cs_incoming_message_save_failed");
  ...
  async function finish(reply: string, paused: boolean, delivered: boolean) {
    const { error: updateError } = await admin.from("kakao_cs_conversations").update({
      messages: delivered ? [...messages, { role: "assistant", content: reply }].slice(-12) : messages.slice(-12),
  ...
  return {
    run: async () => {
      ...
      const response = answer.needs_operator || answer.reply === CS_FALLBACK ? csHandoff(answer.reply) : csText(answer.reply);
      if (!input.callback) {
