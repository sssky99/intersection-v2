...
export function validateMeetingEventOptions(value: Record<string, unknown>): string | null {
  ...
  if (!fee || typeof fee !== "object" || typeof fee.enabled !== "boolean") return "추가요금 설정을 확인해주세요.";
  if (typeof fee.description !== "string" || fee.description.trim().length > 80) return "추가요금 설명은 80자 이내로 입력해주세요.";
  if (typeof fee.amount !== "number" || !Number.isSafeInteger(fee.amount) || fee.amount < 0 || fee.amount > 100000000)
    return "추가요금은 0~100,000,000원의 정수로 입력해주세요.";
  if (fee.enabled && (!fee.description.trim() || fee.amount <= 0)) return "추가요금 설명과 0원보다 큰 금액을 입력해주세요.";
  return null;
}
