...
function buildConversationParagraph(
  ...
  const roleValues = profileQuestions
    .filter((question) => question.category === "모임 역할")
    .flatMap((question) => {
      const answer = answers.find(
        (row) => row.question_order === (question.order ?? question.id),
      );
      return answer?.answer_values ?? (answer?.answer_value ? [answer.answer_value] : []);
    })
    .filter(
      (value): value is "opener" | "connector" | "listener" =>
        value === "opener" || value === "connector" || value === "listener",
    );
  ...
  const dominantRole = (["opener", "connector", "listener"] as const).reduce(
    (best, role) => {
      const count = roleValues.filter((value) => value === role).length;
      return count > best.count ? { role, count } : best;
    },
    { role: roleValues[0], count: 0 },
  ).role;
  const roleCopy = {
    opener: {
      first:
        "처음 만나는 자리에서는 어색한 순간에 먼저 말문을 열고, 조용한 사람에게도 자연스럽게 질문을 건네요.",
      second: `그래서 ${displayName}님도 대화의 시작을 만들고 사람들이 함께 어울리도록 돕는 방식으로 참여해요.`,
    },
    connector: {
      first:
        "처음 만나는 자리에서는 누군가 꺼낸 말을 자연스럽게 이어가고, 주변 사람을 조용히 챙겨요.",
      second: `그래서 ${displayName}님도 반응과 연결로 대화가 편하게 이어지도록 참여해요.`,
    },
    listener: {
      first:
        "처음 만나는 자리에서는 먼저 나서기보다 편하게 듣고 반응하며, 각자의 속도를 존중해요.",
      second: `그래서 ${displayName}님도 차분히 경청하면서 모두가 편하게 머물 수 있도록 참여해요.`,
    },
  } as const;

  return `${roleCopy[dominantRole].first} ${roleCopy[dominantRole].second}`;
}
...
"모임 역할" 카테고리의 세 답변을 함께 보고 사용자가 오프너, 커넥터, 리스너 중 어느 역할에 더 가까운지 행동 중심으로 종합한다.
"온도", "결", "톤", "리듬" 카테고리 답변도 반영해 사용자가 편하게 느끼는 분위기와 대화 스타일을 설명한다.
마지막 문장은 세 역할 답변을 바탕으로, 처음 만나는 자리에서 주로 어떻게 대화에 참여하는지로 마무리한다.
...
publicIntro를 작성할 때 가장 중요한 사실 기반 정보는 workAnswer와 recentInterestAnswers다. 이 두 답변에 들어 있는 구체적인 표현, 관심사, 일하는 방식, 최근 생각하는 주제를 반영한다. conversationAnswers는 사용자의 첫 만남 대화 스타일을 요약하는 데만 사용한다.
...
2문단은 짧게 작성하고 conversationAnswers 중 "모임 역할" 카테고리의 세 답변만 사용한다.
세 답변은 각각 첫 만남에서 대화를 시작하는 방식, 조용한 사람을 대하는 방식, 주변이 보는 대화 모습을 뜻한다. 답변을 함께 보고 오프너, 커넥터, 리스너 중 어느 역할에 더 가까운지 행동 중심으로 종합한다.
2문단에는 온도, 결, 톤, 리듬, 관계 기대, 나이 조건 답변을 사용하지 않는다.
2문단은 반드시 "처음 만나는 자리에서는"으로 시작한다. 사용자가 어떤 방식으로 말문을 열고, 대화를 잇고, 듣고 반응하는지 설명한다. 이 문단은 정확히 두 문장으로 유지한다. 두 번째 문장 중간에는 반드시 "그래서 [publicDisplayName]님도" 형태로 publicDisplayName을 한 번만 쓴다. 내부 역할명인 "오프너", "커넥터", "리스너"를 그대로 노출하지 말고 실제 행동으로 풀어 쓴다. 사용자를 진단하거나 성격 타입으로 축소하지 않는다.
