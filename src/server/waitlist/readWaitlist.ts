...
import "server-only";
import { friendApplicationNames, type WaitlistFriendInvitation } from "./friendApplications";
...
export async function loadWaitlistData(): Promise<AdminWaitlistData> {
  ...
  // Friends may not have applied yet, so the waitlist's profile set is insufficient.
  const friendProfiles: Array<{ user_id: string; phone: string | null; name: string | null }> = [];
  const friendPhones = uniqueText(friendInvitations.map((invitation) => invitation.friend_phone));
  const inviterIds = uniqueText(friendInvitations.map((invitation) => invitation.inviter_id));
  for (const [column, values] of [["phone_normalized", friendPhones], ["user_id", inviterIds]] as const) {
    for (let start = 0; start < values.length; start += 100) {
      const { data, error } = await supabase.from("profiles")
        .select("user_id,name,phone_normalized").is("archived_at", null)
        .in(column, values.slice(start, start + 100));
      if (error) throw error;
      friendProfiles.push(...(data ?? []).map((profile) => ({ user_id: profile.user_id, name: profile.name, phone: profile.phone_normalized })));
    }
  }
  const friendNames = friendApplicationNames(dateApplicationRows, [...profiles, ...friendProfiles], friendInvitations);
  ...
  const dateApplications = dateApplicationRows.map(
    (row): AdminWaitlistRow => {
      ...
      return {
        ...
        is_friend_application: friendNames.has(String(row.id)),
        friend_name: friendNames.get(String(row.id)) ?? null,
        source_id: row.id,
        user_id: row.user_id,
