-- One receipt per applied payment. Existing history is left intact.
create table public.membership_payment_effects (
  transaction_id bigint primary key references public.payment_transactions(id) on delete cascade,
  intent_id bigint not null unique references public.membership_payment_intents(id) on delete cascade,
  ...
  cancellation_revoked_access boolean
);
create unique index membership_payment_effects_current_user_idx
  on public.membership_payment_effects(user_id) where is_current;
create index membership_payment_effects_user_applied_idx on public.membership_payment_effects(user_id,applied_at desc);
alter table public.membership_payment_effects enable row level security;
revoke all on public.membership_payment_effects from public, anon, authenticated;
grant select, insert, update on public.membership_payment_effects to service_role;
...
begin
  ...
  if exists(select 1 from public.groble_webhook_events where merchant_uid=p_merchant_uid
    and event_type='subscription_payment.cancelled') then
...
create function public.cancel_membership_payment(p_transaction_id bigint,p_cancelled_at timestamptz)
returns jsonb language plpgsql security invoker set search_path = '' as $$
declare
...
begin
  ...
  if receipt.cancelled_at is not null then
...
create table public.membership_payment_retry_attempts (
  id bigint generated always as identity primary key,
  event_id text not null references public.groble_webhook_events(event_id) on delete cascade,
  requested_at timestamptz not null default now(),
  response_status integer
);
create index membership_payment_retry_event_idx on public.membership_payment_retry_attempts(event_id);
alter table public.membership_payment_retry_attempts enable row level security;
revoke all on public.membership_payment_retry_attempts from public,anon,authenticated;
grant select,insert,update on public.membership_payment_retry_attempts to service_role;
grant usage,select on sequence public.membership_payment_retry_attempts_id_seq to service_role;
