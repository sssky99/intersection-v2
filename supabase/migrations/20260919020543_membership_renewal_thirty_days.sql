-- Each recurring charge has its own intent and refundable 30-day receipt.
alter table public.membership_payment_intents add column renewal_source_intent_id bigint references public.membership_payment_intents(id);
create index membership_intents_renewal_source_idx on public.membership_payment_intents(renewal_source_intent_id) where renewal_source_intent_id is not null;
alter table public.membership_payment_effects add column extension_days integer;
-- Only backfill receipts whose recorded effect proves a 30-day renewal.
update public.membership_payment_effects e set extension_days=30 from public.payment_transactions t
where t.id=e.transaction_id and t.payment_kind='membership_renewal'
  and (e.after_state->>'end_date')::date-(e.before_state->>'end_date')::date=30;
...
begin
  ...
  if cardinality(transaction_ids)=1 then
    ...
    if txn.user_id is distinct from p_user_id or (txn.membership_payment_intent_id is distinct from p_intent_id and not exists(
         select 1 from public.membership_payment_intents i where i.id=txn.membership_payment_intent_id
           and i.user_id=p_user_id and i.renewal_source_intent_id=p_intent_id))
    ...
    if txn.status in ('cancelled','refunded') then
      return jsonb_build_object('outcome','cancelled','payment_kind',txn.payment_kind,'transaction_id',txn.id,'intent_id',txn.membership_payment_intent_id);
    end if;
    select * into receipt from public.membership_payment_effects where transaction_id=txn.id;
    if found then
      return jsonb_build_object('outcome','already_applied','payment_kind',txn.payment_kind,'transaction_id',txn.id,'intent_id',txn.membership_payment_intent_id);
    end if;
  ...
  -- Reused subscription reference, distinct charge. Do not reuse discounted upgrades.
  if txn.id is null and exists(select 1 from public.payment_transactions where membership_payment_intent_id=p_intent_id) then
    if nullif(btrim(p_merchant_uid),'') is null or intent.status <> 'completed'
       or intent.credit_amount <> 0 or intent.plan <> 'one_month' then
      raise exception 'Membership payment identity conflict; recurring charge requires review';
    end if;
    source_intent_id := intent.id;
    insert into public.membership_payment_intents(user_id,plan,expected_amount,credit_amount,
      status,opened_at,expires_at,renewal_source_intent_id)
    values(p_user_id,intent.plan,intent.expected_amount,0,'superseded',p_paid_at,
      p_paid_at+interval '1 day',source_intent_id) returning * into intent;
  ...
  linked_id := case when source_intent_id is null then intent.meeting_date_application_id end;
  if linked_id is null and source_intent_id is null then
  ...
  if kind='membership_renewal' then
    if profile.membership_end_date is null or profile.membership_start_date is null then
      raise exception 'Renewal has no existing membership dates; review required';
    end if;
    start_date := profile.membership_start_date;
    end_date := profile.membership_end_date + 30;
  else
    base_date := coalesce(app.meeting_date,paid_date);
    start_date := base_date;
    end_date := (base_date + make_interval(months=>months))::date - 1;
  end if;
  renewal_plan := case when kind='membership_renewal' then coalesce(profile.membership_plan,intent.plan) else intent.plan end;
  ...
  after_state := jsonb_build_object('status','active','plan',renewal_plan,'start_date',start_date,'end_date',end_date,'updated_at',p_paid_at);
  ...
  update public.profiles set membership_status='active',membership_plan=renewal_plan,
  ...
  insert into public.membership_payment_effects(transaction_id,intent_id,user_id,merchant_uid,before_state,after_state,extension_days)
    values(txn.id,intent.id,p_user_id,p_merchant_uid,before_state,after_state,case when kind='membership_renewal' then 30 end);
  return jsonb_build_object('outcome','applied','payment_kind',kind,'transaction_id',txn.id,'intent_id',txn.membership_payment_intent_id);
end;
$$;
...
create or replace function public.cancel_membership_payment(p_transaction_id bigint,p_cancelled_at timestamptz)
returns jsonb language plpgsql security invoker set search_path = '' as $$
declare
  txn public.payment_transactions%rowtype; profile public.profiles%rowtype;
  receipt public.membership_payment_effects%rowtype; revoked boolean := false;
begin
  select * into txn from public.payment_transactions where id=p_transaction_id
    and payment_kind in ('membership_initial','membership_upgrade','membership_renewal');
  if not found or p_cancelled_at is null then raise exception 'Membership cancellation not found'; end if;
  perform pg_advisory_xact_lock(hashtextextended('membership:' || coalesce(txn.merchant_uid,txn.provider_event_id),0));
  select * into profile from public.profiles where user_id=txn.user_id for update;
  perform id from public.membership_payment_intents where id=txn.membership_payment_intent_id for update;
  select * into txn from public.payment_transactions where id=p_transaction_id for update;
  select * into receipt from public.membership_payment_effects where transaction_id=txn.id for update;
  if receipt.cancelled_at is not null or txn.status in ('cancelled','refunded') then
    return jsonb_build_object('outcome','already_cancelled','revoked_access',receipt.cancellation_revoked_access);
  end if;
  if txn.payment_kind='membership_renewal' then
    if receipt.extension_days is distinct from 30 or profile.membership_end_date is null then
      raise exception 'Renewal cancellation has no verified 30-day receipt; review required';
    end if;
    revoked := profile.membership_status='active'
      and profile.membership_end_date-30 < (now() at time zone 'Asia/Seoul')::date;
    update public.profiles set membership_end_date=membership_end_date-30,
      membership_status=case when coalesce(revoked,false) then 'expired' else membership_status end,
      membership_updated_at=greatest(membership_updated_at,p_cancelled_at)
      where user_id=txn.user_id;
    update public.payment_transactions set status='cancelled',cancelled_at=p_cancelled_at,updated_at=now() where id=txn.id;
    update public.membership_payment_intents set status='cancelled',ended_at=p_cancelled_at,updated_at=now() where id=txn.membership_payment_intent_id;
    update public.membership_payment_effects set is_current=false,cancelled_at=p_cancelled_at,
      cancellation_revoked_access=coalesce(revoked,false) where transaction_id=txn.id;
    return jsonb_build_object('outcome','cancelled','revoked_access',coalesce(revoked,false),'deducted_days',30);
  end if;
  if receipt.transaction_id is not null then
    revoked := receipt.is_current and profile.membership_updated_at = (receipt.after_state->>'updated_at')::timestamptz
      and profile.membership_status = receipt.after_state->>'status' and profile.membership_plan = receipt.after_state->>'plan'
      and profile.membership_start_date = (receipt.after_state->>'start_date')::date
      and profile.membership_end_date = (receipt.after_state->>'end_date')::date;
  else
    revoked := profile.membership_updated_at = txn.occurred_at
      and not exists(select 1 from public.membership_payment_effects where user_id=txn.user_id and is_current);
  end if;
  if coalesce(revoked,false) then
    update public.profiles set membership_status='cancelled',membership_updated_at=p_cancelled_at where user_id=txn.user_id;
  end if;
  update public.payment_transactions set status='cancelled',cancelled_at=coalesce(cancelled_at,p_cancelled_at),updated_at=now() where id=txn.id;
  update public.membership_payment_intents set status='cancelled',ended_at=coalesce(txn.cancelled_at,p_cancelled_at),updated_at=now()
    where id=txn.membership_payment_intent_id;
  update public.membership_payment_effects set is_current=false,cancelled_at=p_cancelled_at,cancellation_revoked_access=coalesce(revoked,false)
    where transaction_id=txn.id;
  -- Membership refund does not implicitly cancel an independently booked meeting.
  return jsonb_build_object('outcome','cancelled','revoked_access',coalesce(revoked,false));
end;
$$;
revoke all on function public.cancel_membership_payment(bigint,timestamptz) from public,anon,authenticated;
grant execute on function public.cancel_membership_payment(bigint,timestamptz) to service_role;
